...
// --- イントロ & 固定の質問テキスト ---
const INTRO_MESSAGE =
  "本日面接を担当する小林です。よろしくお願いいたします。";
const FIRST_Q = "自己紹介をお願いします。";

// 言語別の概念質問（検知できた言語があれば、その言語に応じて出す）
const CONCEPT_Q = {
  java: "オブジェクト指向についての説明とかできますでしょうか？",
  react: "Reactの仮想DOMと再レンダリングの仕組みを簡潔に説明し、パフォーマンス最適化の具体例を1つ挙げてください。",
  javascript: "JavaScriptのイベントループと非同期処理（microtask / macrotask）の違いを端的に説明してください。",
  typescript: "TypeScriptを導入する利点を型システムの観点から1つ挙げ、簡単な具体例を示してください。",
...
const BACKGROUND_Q =
  "それを開発しようと思った背景を、さらに詳しく教えてください。";

// 条件（チーム言及あり）で出す
const TEAM_ROLE_Q =
  "先ほどチームでの開発に触れていましたが、チーム開発でのあなたの役割を教えてください。";
...
const OTHER_FAVORITES_Q =
  "他に好きな言語や得意な言語があれば教えてください。";
...
const ANY_QUESTIONS_Q =
  "こちらからの質問は以上です。何か質問があればお答えします。何かありますか？";

const FINAL_MESSAGE =
  "ありがとうございました。以上で面接は終了です。お疲れ様でした。";
...
function mentionsTeam(text: string | undefined | null): boolean {
  ...
  return (
    /team/.test(t) ||
    /チーム/.test(kata) ||
    /共同|担当|役割|スクラム|モブ|ペアプロ/.test(kata)
  );
}
...
function detectLanguageKey(answer: string): string | null {
  ...
  // Python
  if (/\bpython\b/i.test(answer) || /パイソン|パイソン/.test(kata)) return "python";
  ...
  if (/\bc言語\b/i.test(kata) || /\bc(?!\w)/i.test(answer)) return "c";
...
// 次の質問を決めるステートマシン
function nextQuestion(session: Session, lastAnswer?: string): { qId: string, text: string, final?: boolean } {
  // 直前回答からフラグ更新
  if (lastAnswer && mentionsTeam(lastAnswer)) {
    session.teamSeen = true;
  }

  // 1. 初回回答後：言語検知して概念質問 or 背景質問
...
function questionTextFromId(qId: string, lang?: string | null): string {
  switch (qId) {
    case "FIRST": return FIRST_Q;
    case "CONCEPT": return conceptQuestionFor(lang ?? null) ?? "";
    case "BACKGROUND": return BACKGROUND_Q;
    case "TEAM_ROLE": return TEAM_ROLE_Q;
    case "WHY_LANGUAGE": return WHY_LANGUAGE_Q(lang);
    case "OTHER_FAVORITES": return OTHER_FAVORITES_Q;
    case "DIFF_LANG": return DIFF_LANG_Q;
    case "GENAI": return GENAI_Q;
    case "STRENGTH": return STRENGTH_Q;
    case "CAREER": return CAREER_Q;
    case "AT_COMPANY": return AT_COMPANY_Q;
    case "ANY_QUESTIONS": return ANY_QUESTIONS_Q;
    case "FINAL": return FINAL_MESSAGE;
    default: return "";
  }
}
